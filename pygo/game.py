from pygo.board import Board
from pygo.utils import Stone, make_2d_array
from pygo.group import Group, GroupManager
from pygo.exceptions import (
...
class Game(object):
    ...
    def __init__(self, config):
        ...
        self.move_count = 0
        # Maximum moves allowed (2 * board size^2)
        self.max_moves = 2 * (self.board_size ** 2)
    ...
    def pass_turn(self):
        ...
        '''
        self.count_pass += 1
        self.move_count += 1  # Count passes as moves

    def is_over(self):
        '''
        Check if the game is over (if there are two consecutive passes or if move limit is reached)
        '''
        # Game ends if there are two consecutive passes or if move limit is reached
        return self.count_pass >= 2 or self.move_count >= self.max_moves
...
class GameUI(object):
    ...
    def play(self):
        ...
        while not self.game.is_over():
            ...
            print(f'Move: {self.game.move_count}/{self.game.max_moves}')
        ...
        if self.game.move_count >= self.game.max_moves:
            print(f'Game ended after reaching the maximum number of moves ({self.game.max_moves}).')
